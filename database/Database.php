...
//get row
public function getRow1($query, $params = []){
	try {
		$stmt = $this->datab->prepare($query);
		$stmt->execute($params);
		return $stmt->fetch();	
...
//get rows
public function getRows1($query, $params = []){
	try {
		$stmt = $this->datab->prepare($query);
		$stmt->execute($params);
		return $stmt->fetchAll();	
...
//insert row
public function insertRow1($query, $params = []){
	try {
		$stmt = $this->datab->prepare($query);
		$stmt->execute($params);
		return TRUE;	
	...
	//<strong>Operator Name: </strong><?php echo $bON; 
 //<br />
	...
	public function Commit(){
		$this->transaction->commit();
			...
			//<strong>Destination: </strong> <br />
	  		 				...
	  		 				//<strong>Capacity: </strong><?php echo $bCap; 
