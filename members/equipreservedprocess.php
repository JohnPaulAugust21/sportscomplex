	...
	if(isset($_POST['bid']))
		{
			...
			if(!$dt){
				...
				';
					...
					{
						 $sql = "SELECT COUNT(schedDate) as rdt FROM rentals WHERE equipmentID = ? AND schedDate = ?";
						...
						if($res['rdt'] > 2)
							{
								echo '
										<div class="alert alert-danger">
										  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
										  <strong>The renting of equipment is already in limit.</strong> 
										</div>
									';
						...
						else
							{
								...
								// $venueRental, $bill 
								$sql = 'SELECT * FROM rentals WHERE equipmentID = ? AND schedDate = ? AND schedTime = ? AND period = ? ';
								...
								if($res)
							...
							
									{
									...
									else
										{
											...
											// $venueRentalFee = $_POST['venueRentalFee'];
											// $bill  = ((int)$fee +(int)$venueRentalFee);
											$sql = " INSERT INTO rentals(schedTime, period, equipmentID, memberID, schedDate)
											VALUES (?,?,?,?,?) ";
...
// ,$fee ,$bill
											...
											echo '
													<div class="alert alert-success">
													  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
													  <strong>Rented Successfully</strong>
													</div>
												';
										}
							}
