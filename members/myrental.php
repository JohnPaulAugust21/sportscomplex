				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse navbar-ex1-collapse">
						<ul class="nav navbar-nav">
							<li><a href="#" style="font-family: Times New Roman; font-size: 20px;">Equipment Rental</a></li>
						</ul>
 ...
 <div class="container">
			...
			if(isset($_GET['delr_id']))
				{
					...
					$sql = "DELETE FROM rentals WHERE memberID = ? AND rentalID = ?";
					$res = $db->deleteRow($sql, [$tid, $delrid]);

						echo '
								<div class="alert alert-success">
								  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								  <strong>Cancel Renting Successfully</strong> 
								</div>
							';
		 ...
		 <br />
		 	 <table id="myTable" class="table table-striped" >  
				<thead>
					...
					<th>DATE</th>
					<!-- <th>TIME</th> -->
					<th>RENTAL BILLING</th>
					<th><center>ACTION</center></th>
				</thead>
				<tbody>
					<?php 
			$tid = $_SESSION['memberID'];
			$sql = "SELECT * FROM equipments e INNER JOIN rentals r ON e.equipmentID = r.equipmentID
			INNER JOIN members m ON r.memberID = m.memberID
			WHERE m.memberID = ? ORDER BY schedDate, equipmentName";
			$res = $db->getRows1($sql, [$tid]);
			...
			foreach ($res as  $r) {
				...
				$r_id = $r['rentalID'];
				$img = $r['equipmentImage'];
				$bn = $r['equipmentName'];
				$bon = $r['equipmentType'];
				//$dstntn = $r['r_dstntn'];
				$bprice = $r['equipmentRentalFee'];
				$rdate = $r['schedDate'];
				$rhr = $r['schedTime'];
				$rampm = $r['period'];
		...
		?>
					<tr>
						...
						<td class="align-img">
							<a class = "btn btn-danger  btn-xs" href="myrental.php?delr_id=<?php echo $r_id; ?>">
								Cancel
